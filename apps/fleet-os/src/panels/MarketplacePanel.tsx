...
import { Globe2, MapPin, Search, Sparkles, Star, X } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-[#030712] bg-noise pb-28 text-white">
      ...
      <div className="mx-auto mt-6 max-w-6xl px-4 sm:px-6">
        ...
        <div className="glass-panel-glow rounded-3xl p-5 backdrop-blur-2xl">
          <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 shadow-inner">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2.5">
            ...
            <div className="ml-auto flex items-center gap-2">
              ...
              <select
                ...
                className="rounded-xl border border-white/10 bg-slate-950/80 px-3 py-1.5 text-xs font-semibold text-cyan-300 outline-none"
              >
        ...
        <div className="mt-5 flex items-center justify-between px-1">
          <p className="text-xs font-semibold uppercase tracking-wider text-slate-400">{t('marketplace.resultCount', { n: results.length })}</p>
          <span className="flex items-center gap-1.5 text-xs text-cyan-300">
            <Sparkles className="h-3.5 w-3.5" /> Luxury Global Mobility Marketplace
          </span>
        </div>

        <div className="mt-3.5 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3" data-testid="marketplace-results-grid">
          {results.map((listing) => {
            ...
            return (
              <motion.div
                ...
                className="glass-panel group relative flex flex-col overflow-hidden rounded-3xl p-5 shadow-2xl transition hover:border-cyan-400/30"
                data-testid="marketplace-listing-card"
              >
                <div className="flex items-center justify-between gap-2">
                  <Badge tone={SOURCE_TONE[listing.source]}>{listing.source}</Badge>
                  <span className="flex items-center gap-1 text-xs font-bold text-amber-300">
                    <Star className="h-3.5 w-3.5 fill-amber-300 text-amber-300" /> {listing.rating.toFixed(1)} <span className="text-slate-400 font-normal">({listing.reviewCount})</span>
                  </span>
                </div>
                <p className="mt-3 text-base font-bold leading-snug text-white group-hover:text-cyan-200 transition">{lang === 'zh' ? listing.titleZh : listing.title}</p>
                
                <div className="mt-2.5 flex items-center gap-2 rounded-xl bg-white/[0.03] p-2 text-xs text-slate-300">
                ...
                <Button size="sm" fullWidth className="mt-3.5" data-testid="marketplace-book-button" onClick={() => handleBook(listing)}>
      ...
      <AnimatePresence>
        {detail && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[1200] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-md" onClick={() => setDetail(null)}>
            <motion.div
              initial={{ opacity: 0, scale: 0.94, y: 16 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              onClick={(e) => e.stopPropagation()}
              className="glass-panel-glow max-h-[85vh] w-full max-w-md overflow-y-auto rounded-3xl p-6 shadow-2xl text-white"
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="flex items-center gap-1 text-xs font-bold text-amber-300">
                    <Star className="h-3.5 w-3.5 fill-amber-300 text-amber-300" /> {detail.rating.toFixed(1)} ({detail.reviewCount})
                  </span>
                </div>
                <button onClick={() => setDetail(null)} className="rounded-full p-1 text-slate-400 hover:bg-white/10 hover:text-white">
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="mt-3 rounded-2xl bg-white/5 p-3">
                ...
                <p className="mt-1 text-sm text-slate-200">{detail.languages.join(' · ')}</p>
              </div>

              <div className="mt-3 rounded-2xl bg-emerald-950/20 border border-emerald-500/20 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-emerald-400">{t('marketplace.inclusions')}</p>
              ...
              <div className="mt-3 rounded-2xl bg-rose-950/20 border border-rose-500/20 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-rose-400">{t('marketplace.exclusions')}</p>
              ...
              <Button fullWidth className="mt-5" onClick={() => handleBook(detail)}>
                {t('marketplace.bookNow')}
              </Button>
            </motion.div>
          </motion.div>
...
function FilterSelect({
  ...
  return (
    <select
      ...
      data-testid={testId}
      className="rounded-xl border border-white/10 bg-slate-950/80 px-3 py-1.5 text-xs font-semibold text-slate-200 outline-none hover:border-cyan-400/40"
    >
      {options.map((o) => (
