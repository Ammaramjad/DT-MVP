  ...
  return (
    <div className="glass-panel rounded-2xl p-3.5" data-testid="driver-stats-header">
      <div className="flex items-center justify-between px-0.5">
        <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">My Order Stats</p>
        <span className="flex items-center gap-1 text-[11px] font-medium text-emerald-300">
          <TrendingUp className="h-3 w-3" /> {completionRate}% completion
        </span>
      </div>
      ...
      <motion.div layout className="mt-2.5 flex items-center justify-between rounded-xl bg-white/[0.03] px-3 py-2 text-[11px]">
        <span className="flex items-center gap-1.5 text-emerald-300">
          <CheckCircle2 className="h-3.5 w-3.5" /> {stats.acceptedToday} accepted today
        </span>
        <span className="flex items-center gap-1.5 text-red-300">
          <XCircle className="h-3.5 w-3.5" /> {stats.declinedToday + stats.missedToday} declined/missed today
        </span>
      </motion.div>
    </div>
  )
