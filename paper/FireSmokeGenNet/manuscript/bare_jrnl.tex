...
\title{FireSmokeGenNet: Boundary-Aware Diffusion Synthesis and Multimodal Quality Filtering for Wildfire-Smoke Monitoring}
%
%
...
\and Kenny Lin,~\IEEEmembership{Senior Engineer, Gama Innovation Inc.}%
...
\end{equation}
where $k\sim\mathcal{U}(10,20)$ denotes the kernel radius and
$N\sim\mathcal{U}(1,3)$ denotes the number of consecutive
operations. At each perturbation step, dilation or erosion is selected
...
\subsection{Distribution Consistency Validation} A key concern in synthetic data generation is whether generated samples follow the statistical characteristics of real smoke observations. If the generated distribution deviates significantly from the real-world distribution, the resulting detection models may fail to generalize in operational forest monitoring environments. To address this issue, we explicitly evaluate the similarity between real and synthetic smoke datasets using statistical distribution metrics. \paragraph{Feature Embedding Extraction} Let: \begin{equation} f_r = \phi(x_r) \end{equation} \begin{equation} f_s = \phi(x_s) \end{equation} where: \[ x_r \in D_{\text{real}} \] \[ x_s \in D_{\text{synthetic}} \] and: \[ \phi(\cdot) \] denotes the feature extractor (ResNet50 backbone). \paragraph{Distribution Similarity Metrics} We compute two complementary metrics: \textbf{Fréchet Inception Distance (FID)} \begin{equation} \text{FID} = \| \mu_r - \mu_s \|^2 + \operatorname{Tr} \left( \Sigma_r + \Sigma_s - 2 (\Sigma_r \Sigma_s)^{1/2} \right) \end{equation} \textbf{Maximum Mean Discrepancy (MMD)} \begin{equation} \text{MMD} = \left\| \frac{1}{n} \sum_{i=1}^{n} \phi(x_i) - \frac{1}{m} \sum_{j=1}^{m} \phi(y_j) \right\|^2 \end{equation} Lower values indicate stronger alignment between real and generated distributions. \paragraph{Interpretation} This validation ensures that the generated smoke samples are not only visually realistic but also statistically consistent with real smoke observations, improving detector robustness under varying environmental conditions.
...
For each of the 8,000 backgrounds $\times$ 3 masks = 24,000 pairs, we generate 4 variations using \method~with different random seeds, yielding 96,000 synthetic smoke images. After VLM filtering, we retain the top 30\% (28,800 samples) for downstream training.
...
FlameDiffuser \cite{wang2024flame} & 27.10 & 0.85 & 0.91 & 0.256 \\
...
FlameDiffuser \cite{wang2024flame} & 32.15 & 0.081 & 6.18 \\
...
Table~\ref{tab:boundary} quantifies boundary quality. \method~produces gradient distributions nearly identical to real smoke (KL divergence 0.041 vs. SD-Inpainting's 0.187), confirming that MRDL successfully models diffuse transitions. The boundary softness $\mathcal{S}$ is also closest to real smoke (0.124 vs. 0.119 for real).
...
\textbf{Real + \method synth} & \textbf{0.829} & \textbf{0.531} & \textbf{0.847} & \textbf{0.763} \\
\bottomrule
\end{tabular}
\end{table}
...
The largest improvement occurs under weather shift, where background ambiguity is strongest—confirming that boundary-aware diffusion synthesis improves robustness in low-contrast conditions. All improvements are statistically significant ($p<0.01$, paired t-test).
...
Table~\ref{tab:ablation} ablates key architectural components. The full model outperforms all variants:
\begin{itemize}
    \item Removing JCA reduces mAP@50 from 0.829 $\rightarrow$ 0.758 (-8.6\%)
    \item Removing MRDL reduces mAP@50 from 0.829 $\rightarrow$ 0.771 (-7.0\%)
    \item Removing ResNet50 (random initialization) reduces mAP@50 from 0.829 $\rightarrow$ 0.735 (-11.3\%)
    \item Removing filtering reduces mAP@50 from 0.829 $\rightarrow$ 0.748 (-9.8\%)
\end{itemize}
...
\paragraph{Filtering Threshold Sensitivity.}
Table~\ref{tab:filter_threshold} reports an exploratory retention-threshold analysis. The 30\% setting obtains the strongest result in this sweep. Because the retained dataset size changes with the threshold, the comparison reflects the combined influence of sample quality and sample quantity and should not be interpreted as an isolated causal estimate of filtering quality.

\begin{table}[t]
\centering
\caption{\textbf{Effect of filtering threshold on detection performance.} YOLOv13 detector.}
...
\paragraph{L1: Fog-Smoke Confusion (5-8\% of failure cases).}
In high-haze conditions, generated smoke occasionally resembles fog rather than distinct smoke plumes. This occurs because the training data contains both fog and smoke examples, and the model sometimes confuses them. Potential solution: incorporate explicit weather conditioning or use contrastive learning to separate fog and smoke representations.

\paragraph{L2: Over-Opacity in High-Contrast Scenes (3-5\% of cases).}
When smoke should be extremely faint (early-stage fires, distant plumes), the model sometimes generates overly opaque outputs. This reflects a bias in the training data toward visible smoke. Potential solution: augment training with more examples of faint smoke or use density prediction as an auxiliary task.
...
\begin{enumerate}
    ...
    \item Mask Random Difference Loss (MRDL), a stochastic boundary regularization that models boundary uncertainty via morphological perturbations, achieving boundary softness (KL divergence 0.041) nearly identical to real smoke.
    \item A multimodal quality filtering pipeline using fine-tuned Qwen2-VL that retains only top 30\% of samples, improving downstream detection mAP@50 by 8.1\% over random selection.
\end{enumerate}
